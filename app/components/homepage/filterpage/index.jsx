...
const TopAnime = () => {
  ...
  const fetchData = async () => {
    const topAnimeRes = await fetch(
      `${process.env.NEXT_PUBLIC_API_BASE_URL}/top/anime?limit=12&filter=${filterHeader}`
    );
    ...
    <div className="lg:my-5">
      ...
      <div className="flex justify-center items-center p-5">
        <button className="bg-black p-4 w-[200px] rounded-md hover:bg-opacity-40 hover:text-blue-300">
          <span className="hover:border-white">
            Show All {filterHeader === "" ? "Top" : filterHeader}
          </span>
        </button>
      </div>
    </div>
